...
namespace N1C_Movement
{
	public class TFMovementCamera : MonoBehaviour
	{
		...
		public IEnumerator TiltCam(int side = 0)
		{
			...
			{
				float newRot = Mathf.LerpAngle(Transform.eulerAngles.z, targetRot, 8f * Time.deltaTime);
		...
		void Start()
		{
			SetFollowTransform(_camFollowPoint);
		}
		...
		[SerializeField] Transform _camFollowPoint;
